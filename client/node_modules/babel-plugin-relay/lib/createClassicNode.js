      ...
      if (directives.length === 0) {
        ...
        switch (directive.name.value) {
          ...
          case 'relay':
            ...
            // $FlowFixMe graphql 0.12.2
            relayArguments[0].name.value === 'mask') ? process.env.NODE_ENV !== 'production' ? require('./invariant')(false, 'BabelPluginRelay: Expected `@relay` directive to only have `mask` argument in ' + 'compat mode, but get %s',
            // $FlowFixMe graphql 0.12.2
...
function createSubstitutionsForFragmentSpreads(t, path, fragments) {
  return Object.keys(fragments).map(function (varName) {
    ...
    if (!fragment.isMasked) {
      !(path.scope.hasBinding(module) || path.scope.hasBinding(propName)) ? process.env.NODE_ENV !== 'production' ? require('./invariant')(false, 'BabelPluginRelay: Please make sure module \'' + module + '\' is imported and not renamed or the\n        fragment \'' + fragment.name + '\' is defined and bound to local variable \'' + propName + '\'. ') : require('./invariant')(false) : void 0;
